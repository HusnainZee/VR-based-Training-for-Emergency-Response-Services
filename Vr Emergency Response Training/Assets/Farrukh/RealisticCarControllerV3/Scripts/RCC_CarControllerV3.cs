...
public class RCC_CarControllerV3 : RCC_Core {
    ...
    public bool externalThrottleControl = false;
    public float externalThrottleInput = 0f;

    public void ApplyThrottle(float throttleInput)
    {
        externalThrottleInput = throttleInput;
        externalThrottleControl = true;
    }
    public bool externalSteeringControl = false;
    public float externalSteerInput = 0f;

    public void ApplyExternalSteering(float steeringInput)
    {
        externalSteerInput = steeringInput;
        externalSteeringControl = true;
    }
    public void ResetThrottleControl()
    {
        externalThrottleControl = false;
        throttleInput = 0f;
    }
    void Awake (){
	...
	private void Inputs() {
		...
		if (canControl) {
			...
			if (!externalController) {
				...
				if(!externalSteeringControl)
				{
                    steerInput = inputs.steerInput;
                }
				else
				{
                    steerInput = externalSteerInput;
                }
                if (externalThrottleControl)
                {
                    throttleInput = externalThrottleInput;
                }
                else
                {
